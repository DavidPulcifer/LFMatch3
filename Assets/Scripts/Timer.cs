...
public class Timer : MonoBehaviour
{
    ...
    public void UpdateTimer(int currenTime)
    {
        if (paused) return;

        if(clockImage != null)
        {
            clockImage.fillAmount = (float) currenTime / (float) m_maxTime;

            if(currenTime <= flashTimeLimit)
            {
        ...
        if(timeLeftText != null)
        {
            timeLeftText.text = currenTime.ToString();
        }
    }
