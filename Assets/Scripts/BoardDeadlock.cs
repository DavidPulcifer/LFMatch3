...
public class BoardDeadlock : MonoBehaviour
{
    ...
    List<GamePiece> GetNeighbors(GamePiece[,] allPieces, int x, int y)
    {
        ...
        foreach (Vector2 dir in searchDirections)
        {
            if (x + (int)dir.x < 0 || x + (int)dir.x >= width ||
                y + (int)dir.y < 0 || y + (int)dir.y >= height) continue;
            if (allPieces[x + (int)dir.x, y + (int)dir.y] == null) continue;
            if (neighbors.Contains(allPieces[x + (int)dir.x, y + (int)dir.y])) continue;
    ...
    bool HasMoveAt(GamePiece[,] allPieces, int x, int y, int listLength = 3, bool checkRow = true)
    {
        ...
        if (unmatchedPiece == null) return false;
        ...
        if (matches.Count < listLength) return false;
    ...
    public bool IsDeadlocked(GamePiece[,] allPieces, int listLength = 3)
    {
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            {
                if(HasMoveAt(allPieces, x, y, listLength, true) || HasMoveAt(allPieces, x, y, listLength, false))
                {
                    isDeadlocked = false;
                }
            }
